...
def get_single_class_cifar100(data_root, class_index):
    ...
    train_indices = [
        i for i, y in enumerate(full_train.targets) if y == class_index
    ]
    test_indices = [
        i for i, y in enumerate(full_test.targets) if y == class_index
    ]
    train_subset = Subset(full_train, train_indices)
    test_subset = Subset(full_test, test_indices)
    return train_subset, test_subset, len(train_indices), len(test_indices)
...
def run_single_experiment(
    ...
    best_acc = 0.0
    log(
        f"Start one-class distill class={class_index} ratio={ratio:.1f} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            log(
                f"[class {class_index}] ratio={ratio:.1f} method={method} "
    ...
    history_dir = os.path.join(args.save_dir, "distill_oneclass_history")
    os.makedirs(history_dir, exist_ok=True)
    history_path = os.path.join(
        history_dir,
        f"cifar100_class{class_index}_ratio{ratio:.1f}_{method}.npz",
    )
    np.savez_compressed(
        history_path,
...
def run_all_experiments(args):
    ...
    train_subset, test_subset, train_count, test_count = get_single_class_cifar100(
        args.data_root,
        args.class_index,
    )
    ...
    if not log_path:
        log_filename = (
            f"distill_cifar100_oneclass_class{args.class_index}_KD-DKD.log"
        )
    ...
    log(
        f"Start CIFAR100 one-class distillation class_index={args.class_index} "
    ...
    all_indices = np.arange(train_count)
    rng.shuffle(all_indices)

    for ratio in ratios:
        count = max(1, int(train_count * ratio))
    ...
    log("All one-class distillation experiments finished")
