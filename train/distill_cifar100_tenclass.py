...
def get_ten_classes_cifar100(data_root, class_indices):
    ...
    for c in class_indices:
        train_class_indices[c] = [
            i for i, y in enumerate(full_train.targets) if y == c
        ]
    test_indices = [
        i for i, y in enumerate(full_test.targets) if y in class_indices
    ]
    test_subset = Subset(full_test, test_indices)
    return full_train, test_subset, train_class_indices
...
def run_single_experiment(
    ...
    best_acc = 0.0
    log(
        f"Start ten-class distill classes={class_indices} ratio={ratio:.1f} "
        f"method={method} samples={len(selected_indices)} epochs={args.epochs}",
    )
    for epoch in range(1, args.epochs + 1):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            log(
                f"[ten-class] ratio={ratio:.1f} method={method} "
    ...
    history_dir = os.path.join(args.save_dir, "distill_tenclass_history")
    os.makedirs(history_dir, exist_ok=True)
    history_path = os.path.join(
        history_dir,
        f"cifar100_tenclass_ratio{ratio:.1f}_{method}.npz",
    )
    np.savez_compressed(
        history_path,
...
def run_all_experiments(args):
    ...
    class_indices = class_indices[:10]
    full_train, test_subset, train_class_indices = get_ten_classes_cifar100(
        args.data_root,
        class_indices,
    )
    log_path = args.log_file
    if not log_path:
        log_filename = "distill_cifar100_tenclass_KD-DKD.log"
    ...
    for c in class_indices:
        indices_c = np.array(train_class_indices[c], dtype=np.int64)
        rng.shuffle(indices_c)
        shuffled_indices_per_class[c] = indices_c
    for ratio in ratios:
        selected_indices_list = []
        for c in class_indices:
            indices_c = shuffled_indices_per_class[c]
            count_c = max(1, int(len(indices_c) * ratio))
            selected_indices_list.append(indices_c[:count_c])
        if not selected_indices_list:
            continue
        selected_indices = np.concatenate(selected_indices_list, axis=0)
    ...
    log("All ten-class distillation experiments finished")
