...
def get_cifar100_by_class(data_root):
...
def run_single_experiment(
    ...
    best_acc = 0.0
    log(
        f"Start CIFAR100 full-class distill ratio={ratio:.1f} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            log(
                f"[full-class] ratio={ratio:.1f} method={method} "
    ...
    history_dir = os.path.join(args.save_dir, "distill_cifar100_full_history")
    os.makedirs(history_dir, exist_ok=True)
    history_path = os.path.join(
        history_dir,
        f"cifar100_full_ratio{ratio:.1f}_{method}.npz",
    )
    np.savez_compressed(
        history_path,
...
def run_all_experiments(args):
    ...
    methods = ["KD", "DKD"]
    full_train, test_subset, train_class_indices = get_cifar100_by_class(
        args.data_root,
    )
    log_path = args.log_file
    if not log_path:
        log_filename = "distil_cifar100_full_KD-DKD.log"
    ...
    for key, value in sorted(vars(args).items()):
        log(f"{key}={value}")
    log(
        f"Start CIFAR100 full-class distillation "
    ...
    log("All CIFAR100 full-class distillation experiments finished")
