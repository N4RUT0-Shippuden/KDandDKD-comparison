...
def plot_student_cifar100_class0(history_dir, output_dir):
    ...
    for ratio in ratios:
        kd_path = os.path.join(
            history_dir,
            f"cifar100_class{class_index}_ratio{ratio:.1f}_KD.npz",
        )
        dkd_path = os.path.join(
            history_dir,
            f"cifar100_class{class_index}_ratio{ratio:.1f}_DKD.npz",
        )
        ...
        ax.set_title(f"CIFAR100 Class 0 Student - Ratio {ratio:.1f}")
        ax.legend()
        fig.tight_layout()
        out_path = os.path.join(
            output_dir,
            f"cifar100_class0_ratio{ratio:.1f}_student_top1.png",
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--history-dir",
        type=str,
        default="./checkpoints/distill_oneclass_history",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./plots/student_cifar100_class0",
    )
...
if __name__ == "__main__":
    args = parse_args()
    plot_student_cifar100_class0(args.history_dir, args.output_dir)
