...
def plot_student_cifar100_allclasses(history_dir, output_dir):
    ...
    for ratio in ratios:
        kd_path = os.path.join(
            history_dir,
            f"cifar100_allclasses_teacher_ratio{ratio:.1f}_KD.npz",
        )
        dkd_path = os.path.join(
            history_dir,
            f"cifar100_allclasses_teacher_ratio{ratio:.1f}_DKD.npz",
        )
        ...
        out_path = os.path.join(
            output_dir,
            f"cifar100_allclasses_ratio{ratio:.1f}_student_top1.png",
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--history-dir",
        type=str,
        default="./checkpoints/distill_cifar100_allclasses_history",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./plots/student_cifar100_allclasses",
    )
...
if __name__ == "__main__":
    args = parse_args()
    plot_student_cifar100_allclasses(args.history_dir, args.output_dir)
