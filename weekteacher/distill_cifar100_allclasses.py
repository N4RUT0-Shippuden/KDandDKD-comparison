...
def get_cifar100_datasets(data_root):
    num_classes = 100
...
def run_single_experiment(
    ...
    best_acc = 0.0
    log(
        f"Start distill CIFAR100 all classes teacher_ratio={ratio:.1f} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            log(
                f"[teacher_ratio={ratio:.1f}] method={method} "
    ...
    history_dir = os.path.join(args.save_dir, "distill_cifar100_allclasses_history")
    os.makedirs(history_dir, exist_ok=True)
    history_path = os.path.join(
        history_dir,
        f"cifar100_allclasses_teacher_ratio{ratio:.1f}_{method}.npz",
    )
    np.savez_compressed(
        history_path,
...
def run_all_experiments(args):
    ...
    train_dataset, test_dataset, num_classes = get_cifar100_datasets(args.data_root)
    ratios = [i / 10.0 for i in range(10, 0, -1)]
    methods = ["KD", "DKD"]
    log_path = args.log_file
    if not log_path:
        log_filename = "distill_cifar100_allclasses_KD-DKD.log"
    ...
    for key, value in sorted(vars(args).items()):
        log(f"{key}={value}")
    log(
        f"Start CIFAR100 all-classes distillation "
    ...
    log("All CIFAR100 all-classes distillation experiments finished")
