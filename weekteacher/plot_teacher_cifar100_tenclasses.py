...
def plot_teacher_cifar100_tenclasses(history_dir, output_dir, classes_str):
    ...
    ax.set_title(f"CIFAR100 Teacher (Ten Classes: {tag}) - Top-1 for Different Ratios")
    ax.legend()
    fig.tight_layout()

    out_path = os.path.join(
        output_dir,
        f"teacher_cifar100_tenclasses_{tag}_Top-1_accs.png",
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./plots/teacher_cifar100_tenclasses",
    )
    parser.add_argument(
        "--classes",
        type=str,
...
if __name__ == "__main__":
    args = parse_args()
    plot_teacher_cifar100_tenclasses(
        args.history_dir,
        args.output_dir,
        args.classes,
    )
